...
void or_regex_node_::generate(std::ostream& os, random_generator_base& random_gen, char& last_gen)
{
  if(or_nodes_.size() == 1)
  {
    random_value_ = 0;
  ...
  or_nodes_[random_value_]->generate(os, random_gen, last_gen);
}

void or_regex_node_::regenerate(std::ostream& os, char& last_gen) const
{
  or_nodes_[random_value_]->regenerate(os, last_gen);
}
